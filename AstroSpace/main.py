...
class Game:
            ...
            if len(self.enemies) < MAX_ENEMIES:
                ...
                if self.recording_pattern and self.current_wave == 1:
                    self.wave_pattern.append({
                        ...
                        'time': pygame.time.get_ticks() - self.wave_start_time
                    })
    ...
    def spawn_wave_enemy(self, pattern_data):
        """Spawn enemy based on recorded pattern with wave modifications"""
        try:
            if len(self.enemies) < MAX_ENEMIES:
                enemy = Enemy(self.assets)
                
                # Apply wave modifications
                if self.current_wave == 2:  # Faster
                    enemy.rect.x = pattern_data['x']
                    enemy.rect.y = pattern_data['y']
                    enemy.speed_x = pattern_data['speed_x'] * 1.5
                    enemy.speed_y = pattern_data['speed_y'] * 1.5
                elif self.current_wave == 3:  # Reversed movement
                    enemy.rect.x = pattern_data['x']
                    enemy.rect.y = pattern_data['y']
                    enemy.speed_x = -pattern_data['speed_x']
                    enemy.speed_y = pattern_data['speed_y']
                elif self.current_wave == 4:  # Mirrored position
                    enemy.rect.x = SCREEN_WIDTH - pattern_data['x']
                    enemy.rect.y = pattern_data['y']
                    enemy.speed_x = -pattern_data['speed_x']
                    enemy.speed_y = pattern_data['speed_y']
                else:  # Default
            ...
            print(f"Error spawning wave enemy: {e}")
    ...
    def update_gameplay(self):
        ...
        # Update enemies (only if time freeze is not active)
        if not self.player.time_freeze_active:
        ...
        # Update powerups (only if time freeze is not active)
        if not self.player.time_freeze_active:
        ...
        # Time loop wave management
        self.manage_time_waves()
    ...
    def manage_time_waves(self):
        """Manage the time loop wave system"""
        current_time = pygame.time.get_ticks()
        wave_duration = 30000  # 30 seconds per wave
        
        # Check if it's time for next wave
        if current_time - self.wave_start_time >= wave_duration:
            self.current_wave += 1
            self.wave_start_time = current_time
            self.recording_pattern = False
            
            # Clear current enemies for clean wave transition
            self.enemies.clear()
            
            # Reset wave after 4 iterations
            if self.current_wave > 4:
                self.current_wave = 1
                self.wave_pattern.clear()
                self.recording_pattern = True
        
        # Replay pattern for waves 2-4
        if not self.recording_pattern and self.wave_pattern:
            for pattern_data in self.wave_pattern:
                if current_time - self.wave_start_time >= pattern_data['time']:
                    # Only spawn if we haven't spawned this pattern entry yet
                    if not hasattr(pattern_data, 'spawned'):
                        self.spawn_wave_enemy(pattern_data)
                        pattern_data['spawned'] = True
            
            # Reset spawned flags when wave restarts
            if current_time - self.wave_start_time < 1000:  # First second of wave
                for pattern_data in self.wave_pattern:
    ...
    def render_instructions(self):
        ...
        instructions = [
            ...
            "• Purple: Double Shot • Blue: Shield",
            "• Yellow: Speed Boost • Red: Extra Life",
            "• Light Blue: Time Freeze (Chrono Crystal)",
            "",
            "Time Loop Waves repeat every 30 seconds",
